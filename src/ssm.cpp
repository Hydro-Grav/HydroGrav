...
PowerSpec::PowerSpec(const std::vector<double>& freq_vals, std::vector<double>& P_vals)
    : freq_vals_(freq_vals), P_vals_(P_vals) {
        if (freq_vals.size() != P_vals.size()) {
...
void PowerSpec::write(const std::string& filename) const {
    ...
    for (size_t i = 0; i < freq_vals_.size(); ++i) {
        file << freq_vals_[i] << "," << P_vals_[i] << "\n";
    }
    file.close();
...
PowerSpec operator*(const PowerSpec& spec, double scalar) {
    ...
    return PowerSpec(spec.freq(), scaled_P);
}
...
PowerSpec PowerSpec::norm_spec() {
    const auto spec_max = peak_vals().second;
    if (spec_max == 0.0) {
        throw std::runtime_error("Division by zero in norm_spec from peak_vals().second = 0");
    } else if (isnan(spec_max)) {
        throw std::runtime_error("In norm_spec: peak_vals().second = nan");
    }

    const auto zk = *this / spec_max;
...
SSM_PowerSpec::SSM_PowerSpec(const std::vector<double>& freq_vals, std::vector<double>& P_vals, const Hydrodynamics::FluidProfile& profile, double dtau)
    : PowerSpec(freq_vals, P_vals),
      K_vals_(freq_to_kRs(freq_vals, *profile.params())),
      dtau_(dtau),
      profile_(profile), params_(profile.params()) {

      if (dtau <= 0.0) {
        throw std::invalid_argument("dtau must be > 0");
      }
    }

SSM_PowerSpec operator*(const SSM_PowerSpec& spec, double scalar) {
    std::vector<double> scaled_P;
    scaled_P.reserve(spec.P().size());
    ...
    return SSM_PowerSpec(spec.freq(), scaled_P, spec.profile(), spec.dtau());
}

SSM_PowerSpec operator*(double scalar, const SSM_PowerSpec& spec) {
    return spec * scalar;
}
...
SSM_PowerSpec SSM_PowerSpec::norm_spec() {
    const auto spec_max = peak_vals().second;
    if (spec_max == 0.0) {
        throw std::runtime_error("Division by zero in norm_spec from peak_vals().second = 0");
    } else if (isnan(spec_max)) {
...
SSM_PowerSpec GWSpec(const std::vector<double>& kRs_vals, const PhaseTransition::PTParams& params, const double dtau) {
    ...
    const auto freq_vals = kRs_to_freq(kRs_vals, params);
    ...
    return SSM_PowerSpec(freq_vals, GW_P_vals, profile, dtau);
}
/***************************/

std::vector<double> kRs_to_freq(const std::vector<double>& kRs_vals, const PhaseTransition::PTParams& params) {
    const auto HsRs = params.un().Hs() * params.Rs();
    const auto fac = (2.6e-5) * (1.0 / 10.0) * (1.0 / HsRs) * (params.un().Ts() / 100.0) * std::pow(params.un().gs() / 100.0, 1.0 / 6.0);
    
    const auto n = kRs_vals.size();
    std::vector<double> freq_vals(n);
    for (size_t i = 0; i < n; i++) {
        freq_vals[i] = kRs_vals[i] * fac;
    }
...
std::vector<double> freq_to_kRs(const std::vector<double>& freq_vals, const PhaseTransition::PTParams& params) {
    const auto HsRs = params.un().Hs() * params.Rs();
    const auto fac = (10.0 * HsRs / 2.6e-5) * (100.0 / params.un().Ts()) * std::pow(100.0 / params.un().gs(), 1.0 / 6.0);
    
    const auto n = freq_vals.size();
    std::vector<double> kRs_vals(n);
    for (size_t i = 0; i < n; i++) {
        kRs_vals[i] = freq_vals[i] * fac;
    }
...
/*** Kinetic spectrum ***/
SSM_PowerSpec Ekin(const std::vector<double>& kRs_vals, const PhaseTransition::PTParams& params) {
    return Ekin(kRs_vals, Hydrodynamics::FluidProfile(params));
}

SSM_PowerSpec Ekin(const std::vector<double>& kRs_vals, const Hydrodynamics::FluidProfile& prof) {
