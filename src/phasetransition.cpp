...
PTParams::PTParams(double vw, double alN) // Fluid profile only uses vw and alN
    : PTParams(vw, alN, dflt_PTParams::beta, dflt_PTParams::dtau, dflt_PTParams::TN, dflt_PTParams::cpsq, dflt_PTParams::cmsq, dflt_PTParams::nuc_type, default_universe()) {}
...
std::ostream& operator<<(std::ostream& os, const PTParams& params) {
    os << "********** Phase Transition parameters **********\n"
      ...
      //  << std::setw(35) << "Equation of state:" << params.model_ << "\n"
       ...
       << std::setw(35) << "Speed of sound (symmetric phase):" << "cpsq=" << params.cpsq_ << "\n"
       << std::setw(35) << "Speed of sound (broken phase):" << "cmsq=" << params.cmsq_ << "\n"
       << "*************************************************\n";
...
/************************************ PTParams ************************************/
PTParams2::PTParams2(double vw, double alN, double TN, double beta, double dtau, const char* nuc_type, const Universe& un)
    : un_(un),
      vw_(vw),
      alN_(alN),
      TN_(TN),
      ...
      if (vw_ < 0.0 ) {
        ...
        throw std::invalid_argument("Unphysical wall velocity passed into PTParams2. Must have 0 < vw < 1.");
      }

      // check valid alN
      if (alN_ <= 0.0) {
        throw std::invalid_argument("Unphysical strength parameter passed into PTParams2. Must have alN > 0.");
      }

      // check valid TN
      if (TN_ <= 0.0) {
        throw std::invalid_argument("Unphysical nucleation temperature passed into PTParams2. Must have TN > 0.");
      }

      // Check valid beta
      if (beta_ <= 0.0) {
        throw std::invalid_argument("Unphysical transition rate parameter passed into PTParams2. Must have beta > 0.");
      }
...
// Private:
bool PTParams2::is_valid_model(const char* model, const char* allowed_models[], const int n) const {
  for (int i = 0; i < n; i++) {
    if (std::strcmp(model, allowed_models[i]) == 0) {
      return true;
    }
...
/********************************** PTParams2_Bag **********************************/
PTParams2_Bag::PTParams2_Bag(double vw, double alN) // Bag model
    : PTParams2_Bag(vw, alN, dflt_PTParams::TN, dflt_PTParams::beta, dflt_PTParams::dtau, dflt_PTParams::nuc_type, default_universe(), dflt_PTParams::cpsq, dflt_PTParams::cmsq) {}

PTParams2_Bag::PTParams2_Bag(double vw, double alN, double TN, double cpsq, double cmsq) // mu nu model
    : PTParams2_Bag(vw, alN, dflt_PTParams::TN, dflt_PTParams::beta, dflt_PTParams::dtau, dflt_PTParams::nuc_type, default_universe(), cpsq, cmsq) {}

PTParams2_Bag::PTParams2_Bag(double vw, double alN, double TN, double beta, double dtau, const char* nuc_type, const Universe& un, double cpsq, double cmsq)
    : PTParams2(vw, alN, TN, beta, dtau, nuc_type, un),
      cpsq_(cpsq),
      cmsq_(cmsq) {

      // check valid speed of sound
      if (!is_valid_csq(cpsq_)) {
        throw std::invalid_argument("Unphysical speed of sound in symmetric phase passed into PTParams2_Bag. Must have 0 < cpsq < 1.");
      }
      if (!is_valid_csq(cmsq_)) {
        throw std::invalid_argument("Unphysical speed of sound in broken phase passed into PTParams2_Bag. Must have 0 < cmsq < 1.");
      }
...
// Public:
void PTParams2_Bag::print() const {
  PTParams2::print();
...
// Private:
bool PTParams2_Bag::is_valid_csq(double csq) const {
  return (csq > 0.0 && csq < 1.0);
}

/********************************** PTParams2_Veff *********************************/
PTParams2_Veff::PTParams2_Veff(double vw, double alN, double TN, const std::string& veff_eos_filename)
    : PTParams2_Veff(vw, alN, TN, dflt_PTParams::beta, dflt_PTParams::dtau, dflt_PTParams::nuc_type, default_universe(), veff_eos_filename) {}

// TO DO: remove cpsq and cmsq from inputs when cs(T) implemented
PTParams2_Veff::PTParams2_Veff(double vw, double alN, double TN, double beta, double dtau, const char* nuc_type, const Universe& un, const std::string& veff_eos_filename)
    : PTParams2(vw, alN, TN, beta, dtau, nuc_type, un),
      cpsq_(1.0 / 3.0),
      cmsq_(1.0 / 3.0) {
      ...
      if (veff_eos_filename.empty()) {
        ...
        throw std::invalid_argument("Must provide veff eos filename to PTParams2_Veff.");
...
// Public:
#ifdef ENABLE_MATPLOTLIB
void PTParams2_Veff::plot_thermo(const std::string& filename) const {
    plt::figure_size(2400, 1600);
...
void PTParams2_Veff::print() const {
  PTParams2::print();
  std::cout << "*************************************************\n";
}


// double PTParams2_Veff::cpsq(double TTN) const {
...
// double PTParams2_Veff::cmsq(double TTN) const {
//   if (TTN < TTN_min() || TTN > TTN_max()) {
//     throw std::out_of_range("Temperature T/TN out of range for Veff eos. Must have " + std::to_string(TTN_min()) + " < T/TN < " + std::to_string(TTN_max()));
//   }
